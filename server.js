...
io.on("connection", (socket) => {
  ...
  socket.on("opt_in_play_again", ({ roomId }) => {
    ...
    // Find the player's name — they could be in players or spectators
    const playerEntry = room.players.find(p => p.id === socket.id);
    const specEntry = (room.spectators || []).find(s => s.id === socket.id);
    const playerName = playerEntry?.name || specEntry?.name || "Player";

    // New room already created — join it as a regular player
    ...
    rooms[newRoomId] = {
      hostId: socket.id,
      players: [{ id: socket.id, name: playerName, isHost: true, vote: null, connected: true, yesCount: 0 }],
      spectators: [],
      questionIndex: 0,
      questionsAsked: 0,
      queue: [],
    ...
    io.to(roomId).emit("play_again_available", { newRoomId, hostName: playerName });
    broadcastOptIns(roomId);
  ...
  // Spectator reverts to player — only allowed in lobby
  socket.on("leave_spectator", ({ roomId }) => {
    ...
    room.players.push({ id: socket.id, name: spec.name, isHost: false, vote: null, connected: true, yesCount: 0 });
    broadcastRoomUpdate(roomId);
    socket.emit("now_playing"); // tell client they're a player again
  });

  // Client reconnects after being away — rejoin their room if possible
  socket.on("rejoin_room", ({ roomId, name, wasSpectator }) => {
    const room = rooms[roomId];
    if (!room || room.state === "over") return;

    // Update socket ID in the right list
    socketRoom[socket.id] = roomId;
    socket.join(roomId);

    if (wasSpectator) {
      // Find by name in spectators and update their socket ID
      const spec = room.spectators.find(s => s.name === name && !s.connected);
      if (spec) {
        spec.id = socket.id;
        spec.connected = true;
        broadcastRoomUpdate(roomId);
      ...
      // Not found — add as new spectator
      room.spectators = room.spectators || [];
      room.spectators.push({ id: socket.id, name, connected: true });
      broadcastRoomUpdate(roomId);
      socket.emit("now_spectating");
    } else {
      // Find by name in players and update socket ID
